...
int main(int argc, char* argv[]) {
   ...
   if (world_rank == 0) {
      ...
      data = NULL; // no hay necesidad de generar datos en otros procesos
   }

   /* Calcular reparto */
   int base = data_count / world_size;
   int resto = data_count % world_size;
   int local_data_count = base + (world_rank < resto ? 1 : 0);

   float* local_data = malloc(local_data_count * sizeof(float));

   /* MPI */
   Count_distributed_data(data, data_count, local_data, local_data_count,
                          bin_maxes, bin_counts, bin_count,
                          min_meas, world_rank, world_size, MPI_COMM_WORLD);
...
/*---------------------------------------------------------------------
 * Function:  Count_distributed_data
 * Purpose:   Distribute data among processes, count local bins,
 *            and reduce to get global bin counts
 * In arg:    argv:  strings from command line
 * Out args:  data:   data to distribute (only in process 0)
 *            data_count:  number of measurements (only in process 0)
 *            local_data_count: number of local measurements (each process)
 *            bin_maxes:   maximum possible value for each bin
 *            bin_count:   number of bins
 *            min_meas:    minimum measurement
 *            world_rank:  rank of the calling process
 *            world_size:  number of processes
 *            comm:        communicator
 * Out args:  bin_counts:  number of elements in each bin (only in process 0)
 *            local_data:  local data array (each process)
 */
void Count_distributed_data(
      float*   data     /* in */, 
      int      data_count  /* in */,
   ...
   for (i = 0; i < world_size; ++i) {
      send_counts[i] = base + (i < resto ? 1 : 0);
      displs[i] = (i == 0) ? 0 : displs[i-1] + send_counts[i-1];
   }

   int my_count = send_counts[world_rank];

   MPI_Scatterv(data, send_counts, displs, MPI_FLOAT, local_data, my_count, MPI_FLOAT, 0, comm);
   ...
   /* Reducir y acumular en proceso 0 */
